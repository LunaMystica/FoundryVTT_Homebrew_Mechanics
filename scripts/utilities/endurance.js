...
async function updateEndurance(target, workflow, brokenTargets = []) {
	...
	if (
		!enduranceItem ||
		enduranceItem.system.uses.spent >= enduranceItem.system.uses.max
	)
...
async function calculateEnduranceReduction(item) {
	...
	switch (item.type) {
		...
		case 'spell':
			if (
				item.name === 'Elemental Bullet' ||
				item.name === 'Sacred Bolt'
			) {
				reduction = 5;
			} else {
				reduction = 40;
 ...
 */
async function processBrokenTargets(
	brokenTargets,
	workflow,
	damageTypeFeatures
) {
	const groupedBrokenTargets = brokenTargets.reduce(
		(acc, { target, damageType }) => {
			if (!acc[damageType]) {
				acc[damageType] = [];
			}
	...
	for (const [damageType, targets] of Object.entries(groupedBrokenTargets)) {
		...
		const totalDamage = workflow.damageRolls
			.filter((damage) => damage.options.type === damageType)
			.reduce((acc, damage) => acc + damage.total, 0);

		let activity = sourceItem.system.activities.values().next().value;

		let activityData = await activityUtils.withChangedDamage(
			activity,
			`${totalDamage}`
		);
